...
for (const n of drawn) {
    ...
    for (const index in boards) {
        ...
        if (position !== undefined) {
            ...
            if ((markings[index].r.includes(5) || markings[index].c.includes(5)) && !(bingoBoardsSoFar.includes(index))) {
                console.log(`BINGO ${n} - board ${index}`);
                bingoCalled = index;
                bingoBoardsSoFar.push(bingoCalled);
                lastCalledNumber = n;
                boardSnapshot = {};
                boardSnapshot = {...boardsNumbers[index]};
            }
        }
    }
    /*
...
let sumNotCalled = 0;
for (const key of Object.keys(boardSnapshot)) {
    if (!boardSnapshot[key]) sumNotCalled += parseInt(key);
}
